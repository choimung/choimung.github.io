...
객체는 다른 객체와 메세지를 주고 받으면서 협력한다. 객체는 메시지를 받으면 객체 그에 따른 로직(행동)을 수행하게 되고, 필요하다면 객체 스스로 내부의 상태값을 변경하게된다. 즉 객체 스스로 일을 하도록 한다.
...
public class Person {
    ...
    // 로직을 직접 수행하는 메서드가 아닌, 상태를 변경하는 메서드가 아닌 getter만을 가진 객체
